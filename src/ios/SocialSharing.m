  ...
  [self.commandDelegate runInBackground:^{
    ...
    for (NSString* filename in filenames) {
      ...
      if (image != nil) {
        ...
        shared = true;
        NSURL *video_url =[NSURL URLWithString: filename];
        ...
        if([[NSFileManager defaultManager] fileExistsAtPath:[video_url absoluteString]]) {
          [[[ALAssetsLibrary alloc] init] writeVideoAtPathToSavedPhotosAlbum:video_url completionBlock:^(NSURL *assetURL, NSError *error) {
            ...
            if(assetURL) {
              ...
              CDVPluginResult * pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"something wrong"];
              [self.commandDelegate sendPluginResult:pluginResult callbackId:self.command.callbackId];
            }
          }];
